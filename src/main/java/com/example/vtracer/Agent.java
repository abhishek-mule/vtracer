...
public class Agent {
  ...
  private static VTracerConfig loadConfig() {
    ...
    try (FileInputStream input = new FileInputStream("vtracer-config.yml")) {
      ...
      System.out.println(
          "[vtracer] Config file not found or invalid, using defaults: " + e.getMessage());
  ...
  private static void startInstrumentation(Instrumentation inst) {
    System.out.printf(
        "[vtracer] Agent loaded – sampling rate: %.0f%%, pinning detection: %b%n",
      ...
      rs.onEvent(
          ...
          event -> {
            if (MethodTimer.tracingEnabled) {
              ...
              String threadName =
                  (recordedThread != null) ? recordedThread.getJavaName() : "unknown-thread";
              System.out.printf(
                  "[vtracer] ⚠️ PINNING DETECTED! Thread: %s, Duration: %d ns%n",
                  threadName, durationNs);
  ...
  public static class VTracerConfig {
    ...
    public static class Instrumentation {
      ...
      public List<String> excludePackages =
          List.of("java.", "sun.", "jdk.internal", "net.bytebuddy");
    }
  ...
  public static class MethodTimer {
    ...
    private static final ConcurrentHashMap<String, Double> methodHistory =
    ...
    public static void exit(@Advice.Enter long startTime, @Advice.Origin String methodName) {
      ...
      System.out.printf(
          "[vtracer] [sampled] Method %s executed in %.2f ms%n", methodName, durationMs);
