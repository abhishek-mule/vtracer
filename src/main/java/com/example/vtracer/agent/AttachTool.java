 ...
 *
 * Usage:
 *   java -cp vtracer.jar com.example.vtracer.agent.AttachTool <pid> [options]
 */
public class AttachTool {

    public static void main(String[] args) {
        if (args.length == 0) {
            System.out.println("Usage: java -cp vtracer.jar com.example.vtracer.agent.AttachTool <pid|list> [options]");
    ...
    private static void attachToJVM(String pid, String agentArgs) {
        try {
            ...
            String agentPath = AttachTool.class.getProtectionDomain()
                    .getCodeSource()
                    .getLocation()
                    .getPath();
