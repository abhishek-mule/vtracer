 ...
 *
 * Handles:
 * - Multi-threaded traces (separate trees per thread)
 * - Unmatched exits (exception unwinding)
 * - Recursion detection
 */
public class CallTreeAnalyzer {
    ...
    public CallTree buildTree(List<TraceEvent> events) {
        ...
        for (TraceEvent event : events) {
            eventsByThread.computeIfAbsent(event.getThreadId(), k -> new ArrayList<>())
                    .add(event);
        }
    ...
    private void buildThreadTree(CallTree tree, long threadId, List<TraceEvent> events) {
        ...
        for (TraceEvent event : events) {
            if (event.isEnter()) {
                CallTree.Node node = new CallTree.Node(
                ...
                if (stack.isEmpty()) {
                    ...
                    System.err.printf("[VTracer] Unmatched EXIT for %s at %d%n",
                ...
                if (!node.getMethodSignature().equals(event.getMethodSignature())) {
                    System.err.printf("[VTracer] Method mismatch: expected %s, got %s%n",
        ...
        if (!stack.isEmpty()) {
            System.err.printf("[VTracer] %d unmatched ENTER events in thread %d%n",
                    stack.size(), threadId);
        }
    }
    ...
    public static class CallTree {
        ...
        public static class Node {
            ...
            public Node(String methodSignature, String className, String methodName,
